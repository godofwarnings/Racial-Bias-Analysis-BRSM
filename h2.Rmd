...
# Tests For Arrest Rate
```{r}
data <- read.csv("/home/radhikesh/Documents/iiith/sem6/BRSM/Project/data_pp/results_arrests_stack_ratio_h3.csv")
summary(data)

data$Subject_Race <- factor(data$Subject_Race,
                         levels=unique(data$Subject_Race))

data$Arrest_by_Stop <- floor(data$Arrest_by_Stop*100)
# This is done becuase Chi-square test is not meant for ratios. so we convert it to numbers.

data

contingency_table <- matrix(data = c(data$Arrest_by_Stop), ncol = 16, byrow = TRUE)
contingency_table
colnames(contingency_table) <- c('CT', 'RI', 'CA', 'AZ', 'WI', 'NC', 'CO', 'LA', 'MD', 'VT', 'PA', 'TN', 'MA')
